...
def _write_crossbuild_file(file):
    gcc_path = cmdutil.which(toolchain.tool_name('gcc'))
    system_root_path = path.dirname(path.dirname(gcc_path))
    crossbuild_root_path = cmdutil.to_cmake_path(
        path.join(system_root_path, toolchain.host_triplet))
